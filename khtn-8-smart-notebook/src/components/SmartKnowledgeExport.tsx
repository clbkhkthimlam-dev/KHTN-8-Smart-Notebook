...
export const SmartKnowledgeExport: React.FC<SmartKnowledgeExportProps> = ({
  ...
  const filteredLessons = useMemo(() => {
    return ALL_LESSONS.filter((l) => {
      const matchDiscipline = selectedDiscipline === "all" || (l.discipline || getLessonDiscipline(l.id)) === selectedDiscipline;
      ...
      return (
        ...
        String(l.id) === query ||
        `bài ${l.id}`.includes(query) ||
        (l.summary && l.summary.toLowerCase().includes(query)) ||
        (l.coreConcepts && l.coreConcepts.some(c => c.toLowerCase().includes(query)))
      );
    });
  }, [selectedDiscipline, lessonSearchQuery]);
        ...
        <div className="lg:col-span-5 space-y-5">
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs space-y-4">
            ...
            {selectionMode === "lessons" && (
              <div className="space-y-3 max-h-[420px] flex flex-col pr-1">
                {/* Search Box */}
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Tìm theo tên bài, số bài, khái niệm..."
                    className="w-full pl-8 pr-3 py-1.5 text-xs bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-cyan-500 focus:border-cyan-500 text-slate-800"
                  />
                  {lessonSearchQuery && (
                    <button
                ...
                <div className="flex items-center gap-1.5 flex-wrap">
                  <button
                    onClick={() => setSelectedLessons(ALL_LESSONS.map(l => l.id))}
                    className="px-2 py-1 bg-cyan-50 text-cyan-800 hover:bg-cyan-100 border border-cyan-200 rounded text-[10px] font-bold transition-all"
                  >
                    + Tất cả 47 bài
                  </button>
                  <button
                    onClick={() => {
                      const physicsIds = ALL_LESSONS.filter(l => (l.discipline || getLessonDiscipline(l.id)) === "physics").map(l => l.id);
                      setSelectedLessons(physicsIds);
                    }}
                    className="px-2 py-1 bg-blue-50 text-blue-800 hover:bg-blue-100 border border-blue-200 rounded text-[10px] font-semibold transition-all"
                  >
                    Vật lí ({ALL_LESSONS.filter(l => (l.discipline || getLessonDiscipline(l.id)) === "physics").length})
                  </button>
                  <button
                    onClick={() => {
                      const chemIds = ALL_LESSONS.filter(l => (l.discipline || getLessonDiscipline(l.id)) === "chemistry").map(l => l.id);
                      setSelectedLessons(chemIds);
                    }}
                    className="px-2 py-1 bg-amber-50 text-amber-800 hover:bg-amber-100 border border-amber-200 rounded text-[10px] font-semibold transition-all"
                  >
                    Hóa học ({ALL_LESSONS.filter(l => (l.discipline || getLessonDiscipline(l.id)) === "chemistry").length})
                  </button>
                  <button
                    onClick={() => {
                      const bioIds = ALL_LESSONS.filter(l => (l.discipline || getLessonDiscipline(l.id)) === "biology").map(l => l.id);
                      setSelectedLessons(bioIds);
                    }}
                    className="px-2 py-1 bg-emerald-50 text-emerald-800 hover:bg-emerald-100 border border-emerald-200 rounded text-[10px] font-semibold transition-all"
                  >
                    Sinh học ({ALL_LESSONS.filter(l => (l.discipline || getLessonDiscipline(l.id)) === "biology").length})
                  </button>
                  <button
                    onClick={() => setSelectedLessons([])}
                    className="px-2 py-1 bg-slate-100 text-slate-600 hover:bg-slate-200 rounded text-[10px] font-semibold transition-all ml-auto"
                  >
                    Bỏ chọn
                  </button>
                </div>
                ...
                <div className="flex items-center justify-between text-[11px] text-slate-500 pt-1 border-t border-slate-100">
                  <span>{filteredLessons.length} bài hiển thị • <strong className="text-cyan-700">{selectedLessons.length} bài đã chọn</strong></span>
                  <button
                    onClick={() => setSelectedLessons(Array.from(new Set([...selectedLessons, ...filteredLessons.map(l => l.id)])))}
